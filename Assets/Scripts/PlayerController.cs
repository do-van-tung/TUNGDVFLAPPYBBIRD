	...
	void LateUpdate () {
		...
		if (GameManager.Instance.GameState ()) {
			if (Input.GetMouseButtonDown (0)) {
				...
				// TungDV
				playerRigid.velocity = (Vector2.down * 9.8f);
				mTimeStop = 0.4f;
		...
		//}
		// Move without gravityScale and AddForce by TungDV 
		if (mTimeStop > 0)
        {
			playerRigid.velocity = (Vector2.up * 9.8f * 1.5f * mTimeStop);
			mTimeStop -= Time.deltaTime;
        }
        else
        {
			// Player Move Down By TungDv
			playerRigid.velocity = (Vector2.down * 9.8f * 0.6f);
			tiltSmooth = maxTiltSmooth;
		}
	}
